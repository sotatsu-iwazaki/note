...
function Draggable(props: PlateElementProps) {
  ...
  const insertSlashCommandLine = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      event.preventDefault();
      event.stopPropagation();

      const elementPath = editor.api.findPath(element) ?? path;

      if (!elementPath) return;

      editor.tf.withoutNormalizing(() => {
        editor.tf.focus();
        editor.tf.select(editor.api.end(elementPath));
        ...
        <Gutter>
          ...
          >
            ...
            >
              <Button
                variant="ghost"
                className="-left-6 absolute h-6 w-6 p-0"
                style={{ top: `${dragButtonTop + 3}px` }}
                onClick={insertSlashCommandLine}
